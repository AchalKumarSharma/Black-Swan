...
export const DynamicVisualCard: React.FC<DynamicVisualCardProps> = ({
  ...
  const seriesColorMap: Record<string, string> = {
    ...
    benchmark: "#6b4d3a",  // Dark Sepia
  };

  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="rounded border border-[#6b4d3a]/40 bg-parchment-light p-2.5 font-sans text-xs shadow-none">
  ...
  return (
    <section className="rounded-md border border-[#6b4d3a]/30 bg-[#f4f0e8] p-6 mb-6 shadow-none transition-all">
      ...
      <div className="relative h-[320px] w-full rounded border border-[#6b4d3a]/30 bg-parchment-light/30 p-2 overflow-hidden">
